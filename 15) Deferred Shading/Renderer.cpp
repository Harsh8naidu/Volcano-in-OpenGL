...
Renderer::Renderer(Window& parent) : OGLRenderer(parent) {
	...
	for (int i = 0; i < LIGHT_NUM; ++i) {
		...
		l.SetPosition(Vector3(rand() % (int)heightmapSize.x, 0.0f, rand() % (int)heightmapSize.z));
...
void Renderer::DrawPointLights() {
	...
	glUniformMatrix4fv(glGetUniformLocation(pointlightShader->GetProgram(), "inverseViewProj"), 1, false, invViewProj.values);
	UpdateShaderMatrices();
	for (int i = 0; i < LIGHT_NUM; ++i) {
		Light& l = pointLights[i];
		SetShaderLight(l);
